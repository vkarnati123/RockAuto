...
type ViewMode = 'list' | 'detail' | 'compare' | 'cart' | 'advancedSearchForm' | 'advancedSearchResults' | 'login' | 'signup' | 'forgotPassword' | 'garage' | 'welcome' | 'checkoutContact' | 'checkoutPayment' | 'orderConfirmation';
...
export default function App() {
  ...
  const handleShowAdvancedSearch = () => {
    setViewMode('advancedSearchForm');
  };

  const handleAdvancedSearch = (filters: SearchFilters) => {
    setAdvancedSearchFilters(filters);
    setViewMode('advancedSearchResults');
    window.scrollTo(0, 0);
  };
  ...
  const handleCloseAdvancedSearchForm = () => {
    setViewMode('list');
    window.scrollTo(0, 0);
  };
  ...
  // Show advanced search form modal
  const showAdvancedSearchForm = viewMode === 'advancedSearchForm';

  // Show advanced search results view
  if (viewMode === 'advancedSearchResults') {
    return (
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7] flex flex-col">
      ...
      {/* Advanced Search Form Modal */}
      {showAdvancedSearchForm && (
        <AdvancedSearchForm
          onBack={handleCloseAdvancedSearchForm}
